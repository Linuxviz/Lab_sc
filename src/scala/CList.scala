...
class CList {
  ...
  def sort(f: (String, String) => Boolean): Boolean = {
    if (iter.current != null) {
  ...
  def input_in_end(string: String): Unit = {
    if (iter.head != null) {
      ...
      val new_elem = new CListContainer(null, string)
      new_elem.set_next(new_elem)
      iter.head = new_elem
  ...
  def save_in_file(path: String = ""): Boolean = {
